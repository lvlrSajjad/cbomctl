...
## This tool's own supply chain

`release.yml` attaches a CycloneDX SBOM of `cbomctl` to every release it cuts,
as `cbomctl-<version>.cdx.json`, generated by
[cyclonedx-py](https://github.com/CycloneDX/cyclonedx-python) from a clean
install of the wheel being published. It is stamped as a **build** SBOM,
because which kind it is decides what it asserts: it names the versions this
project's dependency ranges resolved to, on one Python and one platform,
during that build. Nothing inspects the wheel's bytes, and no component
carries a hash — so it is not evidence of what your own install resolved.

It is an SBOM and not a CBOM, deliberately. No module in `src/` imports
`hashlib`, `hmac`, `secrets`, `ssl` or `cryptography`: `cbomctl` reads
cryptographic inventories and performs none itself, so a CBOM of it would be
an empty `components` array with a CycloneDX header on it. That reads as
evidence and is not. `scripts/check_stats.py` derives the import count from
`src/`, so the sentence above fails the build rather than going quietly stale.
